defmodule ExAxos.RepresentativesAPI do
  @moduledoc """
  Provides API endpoint related to representatives api
  """

  @default_client ExAxos.Client

  @doc """
  Get Representatives

  Get a list of representatives visible to the user

  ## Options

    * `limit`: limit
    * `offset`: offset
    * `sortBy`: sortBy

  """
  @spec list_representatives_using_get(keyword) ::
          {:ok, ExAxos.RepresentativesResultDigest.t()} | :error
  def list_representatives_using_get(opts \\ []) do
    client = opts[:client] || @default_client
    query = Keyword.take(opts, [:limit, :offset, :sortBy])

    client.request(%{
      args: [],
      call: {ExAxos.RepresentativesAPI, :list_representatives_using_get},
      url: "/rest/representative/v1/representatives",
      method: :get,
      query: query,
      response: [
        {200, {ExAxos.RepresentativesResultDigest, :t}},
        {401, :null},
        {403, :null},
        {404, :null}
      ],
      opts: opts
    })
  end
end
